...
class db:
    ...
    def transaction_init():
        ...
        data_frames = [pd.read_csv(os.path.join(src, filename), index_col=0) for filename in os.listdir(src) if filename.endswith('.csv')]
        transactions = pd.concat(data_frames, ignore_index=True)
